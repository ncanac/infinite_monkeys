...
counts = np.zeros((27,27,27,27,27))
...
for i in range(27):
    for j in range(27):
        for k in range(27):
            for l in range(27):
                total = sum(counts[i,j,k,l,:])
                cdfij = counts[i,j,k,l,:]/total if total > 0 else counts[i,j,k,l,:]
                cdf[i,j,k,l,0] = cdfij[0]
                for m in range(1,27):
                    cdf[i,j,k,l,m] = cdfij[m]+cdf[i,j,k,l,m-1]

np.save('./data files/'+author+'5',cdf)
