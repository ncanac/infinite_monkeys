...
def main():
    ...
    for i in range(2,7):
        log = open('./time/time_zeroth_' + str(i),'w')
        for j in range(N):
            log.write(str(monkey(0,i,'twain','twain'))+'\n')
        log.close()
    
    for i in range(2,9):
        log = open('./time/time_first_' + str(i),'w')
        for j in range(N):
            log.write(str(monkey(1,i,'twain','twain'))+'\n')
        log.close()
    
    for i in range(2,12):
        log = open('./time/time_second_' + str(i),'w')
        for j in range(N):
            log.write(str(monkey(2,i,'twain','twain'))+'\n')
        log.close()
    
    for i in range(3,16):
        log = open('./time/time_third_' + str(i),'w')
        for j in range(N):
            log.write(str(monkey(3,i,'twain','twain'))+'\n')
        log.close()
    
    for i in range(4,18):
        log = open('./time/time_fourth_' + str(i),'w')
        for j in range(N):
            log.write(str(monkey(4,i,'twain','twain'))+'\n')
        log.close()
    
    for i in range(5,20):
        log = open('./time/time_fifth_' + str(i),'w')
        for j in range(N):
            log.write(str(monkey(5,i,'twain','twain'))+'\n')
        log.close()
